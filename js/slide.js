const slide = {
    ...
    zoomDezoomElement: function(element, zoom) {
        ...
        // ça va nous aider à connaitre l'état de l'élément courant
    ...
    handleKeyDown: function(event) {
        ...
        if (element != null) { 
            ...
            if (slide.isTransitionTerminated) {                
                if ((event.keyCode == 37 || event.keyCode == 38) && !element.classList.contains('first')) {  
                    // flèche gauche ou haut -> slide précédente            
                    slide.previousSlide(element);
                }
                if ((event.keyCode == 39 || event.keyCode == 40) && !element.classList.contains('last')) {
